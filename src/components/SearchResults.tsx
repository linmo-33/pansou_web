    ...
    <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-4 sm:mb-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl ${color} flex items-center justify-center shadow-lg`}>
          ...
          <div>
            ...
            <span className="text-xs sm:text-sm text-gray-500">{links.length} 个资源</span>
          </div>
        </div>
        <div className={`min-w-[50px] sm:min-w-[60px] h-[50px] sm:h-[60px] rounded-xl sm:rounded-2xl ${color} flex items-center justify-center shadow-lg`}>
      ...
      <div className="space-y-3 sm:space-y-4">
        {links.slice(0, displayLimit).map((link, index) => (
          <div key={index} className="group bg-gradient-to-r from-white to-gray-50 hover:from-blue-50 hover:to-purple-50 border border-gray-200 rounded-lg sm:rounded-xl p-3 sm:p-5 transition-all hover:shadow-md hover:border-blue-300">
            {link.note && (
              ...
              {link.images && link.images.length > 0 && (
                <div className="flex gap-1.5 sm:gap-2">
                  ...
                  {link.images.length > 3 && (
                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gray-100 rounded-lg border-2 border-gray-200 flex items-center justify-center">
      ...
      {links.length > displayLimit && (
        <div className="mt-6 text-center">
          ...
          >
            ...
            <span>加载更多</span>
            <span className="px-2 py-0.5 bg-blue-100 group-hover:bg-white/20 text-blue-600 group-hover:text-white rounded-full text-xs font-bold">
              {links.length - displayLimit}
            </span>
          </button>
        </div>
